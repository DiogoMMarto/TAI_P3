...
def generate_signature(audio_path: Path, signature_path: Path, verbose: bool = False,
    ...
    if verbose:
        cmd.insert(1, "-v")
    log("INFO",f"Running GetMaxFreqs: {' '.join(cmd)}")
...
def extract_segment(input_audio_path: Path, output_segment_path: Path,
    ...
    log("INFO",f"Running SoX (extract_segment): {' '.join(cmd)}")
    try:
...
def convert_audio_format(input_audio_path: Path, output_audio_path: Path) -> bool:
    ...
    log("INFO",f"Running SoX (convert_audio_format): {' '.join(cmd)}")
    try:
        subprocess.run(cmd, check=True, capture_output=True, text=True)
        return True
    except subprocess.CalledProcessError as e:
...
def add_noise(input_audio_path: Path, output_noisy_audio_path: Path,
    ...
    """
    Adds noise to an audio file using SoX.
    ...
    if not output_noisy_audio_path.parent.exists():
        ...
        return False
    try:
        info_cmd = [str(config.SOX_EXE), "--i", "-D", str(input_audio_path)]
        duration_str = subprocess.run(info_cmd, check=True, capture_output=True, text=True).stdout.strip()
        duration = float(duration_str)
    except Exception as e:
        log("ERROR",f"Could not get duration for {input_audio_path}: {e}")
        return False

    temp_noise_file = config.TEMP_DIR / f"temp_noise_{input_audio_path.stem}.wav"
    cmd_gen_noise = [
        str(config.SOX_EXE), "-n", str(temp_noise_file), "synth", str(duration), noise_type, "vol", str(noise_level)
    ]
    cmd_mix_noise = [
    ...
    try:
        ...
        if temp_noise_file.exists():
            temp_noise_file.unlink() # Clean up temp noise file
        return True
    except subprocess.CalledProcessError as e:
        log("ERROR",f"Error running SoX for noise addition on {input_audio_path}:")
        log("ERROR","STDERR:", e.stderr)
        if temp_noise_file.exists(): # Clean up if error occurred after creation
...
def process_audio_file(
    ...
    if signature_path.exists():
        log("INFO",f"Signature already exists for {input_audio_path}. Skipping processing.")
        return True
